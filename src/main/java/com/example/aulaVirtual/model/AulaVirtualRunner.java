...
public class AulaVirtualRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        UsersEntity user1= new UsersEntity(null);
        UsersEntity user2= new UsersEntity(null);
        ...
        userStudent.setUser(user1);
        userStudentRepo.save(userStudent);

        UserTeacherEntity userTeacher= new UserTeacherEntity(null,"pepeLu","qwerty","12345","12345");
        userTeacher.setUser(user2);
        userTeacher.setTeacher(teacher);


        Optional<UserStudentEntity> user= userStudentRepo.findByUsername("20153261");
        if (user.isPresent()){
            System.out.println("Se encotnro");
        }else{
